const langTab = document.getElementById("langTab");
      ...
      const langRecommendedSection = document.getElementById("langRecommendedSection");
      const currRecommendedSection = document.getElementById("currRecommendedSection");
      ...
      langRecommendedItems.forEach(item => {
        item.addEventListener("click", function() {
          ...
          const mainListItems = languageList.querySelectorAll(".item, .item-selected");
          let matchingItem = null;
          for (let i = 0; i < mainListItems.length; i++) {
            if (mainListItems[i].dataset.language === language && mainListItems[i].dataset.country === country) {
      ...
      currRecommendedItems.forEach(item => {
        item.addEventListener("click", function() {
          ...
          // Find matching item in main list
          const mainListItems = currencyList.querySelectorAll(".item, .item-selected");
          let matchingItem = null;
          for (let i = 0; i < mainListItems.length; i++) {
            if (mainListItems[i].dataset.currency === currency && mainListItems[i].dataset.symbol === symbol) {
              matchingItem = mainListItems[i];
              break;
            }
          }
